"""Offline evaluator for long-only liquid-winners momentum V1.
...
from octobot.ai_strategy_lab import (
    liquid_market_timeseries_momentum_v1_research as market_tools,
)
from octobot.ai_strategy_lab import liquid_winners_momentum_v1 as protocol_module


SCHEMA_VERSION = 1
UTC = datetime.timezone.utc
DataQualityError = market_tools.DataQualityError
...
def _load_protocol(path_value: typing.Union[str, pathlib.Path]) -> dict:
    ...
    if persisted != expected:
        raise ValueError("liquid-winners momentum protocol is not frozen")
    return persisted
...
def _load_market(
    ...
    return market_tools._load_market(snapshot_value, history_value)


def _source_artifacts(test_path: pathlib.Path) -> list[dict]:
    parent = market_tools.parent_research
    values = (
        ("evaluator", pathlib.Path(__file__).resolve()),
        ("protocol", pathlib.Path(protocol_module.__file__).resolve()),
        ("test", test_path.resolve()),
        ("quote_volume_loader", pathlib.Path(market_tools.__file__).resolve()),
        ("parent_market_loader", pathlib.Path(parent.__file__).resolve()),
        ("market_loader", pathlib.Path(parent.market_source.__file__).resolve()),
        ("frozen_source_loader", pathlib.Path(parent.source.__file__).resolve()),
    )
    artifacts = []
...
def is_rebalance_date(date: datetime.date) -> bool:
    return (
        (date - protocol_module.REBALANCE_ANCHOR).days
        % protocol_module.HOLDING_DAYS
        == 0
    )


def prepare_market(market: dict) -> dict:
    """Precompute causal eligibility, Amihud values and formation returns."""
    ...
    if (
        ...
        raise DataQualityError("unexpected liquid-winners market shape")
    close_valid = numpy.isfinite(closes) & (closes > 0)
    volume_valid = numpy.isfinite(volumes) & (volumes > 0)
    interval_valid = numpy.zeros_like(close_valid)
    interval_valid[1:] = close_valid[1:] & close_valid[:-1] & volume_valid[1:]
    daily_amihud = numpy.zeros_like(closes)
    valid_rows, valid_columns = numpy.nonzero(interval_valid)
    daily_amihud[valid_rows, valid_columns] = numpy.abs(
        closes[valid_rows, valid_columns]
        / closes[valid_rows - 1, valid_columns]
        - 1.0
    ) / volumes[valid_rows, valid_columns]
    ...
    interval_invalid_cumulative = numpy.vstack(
        (
            numpy.zeros((1, closes.shape[1]), dtype=numpy.int64),
            numpy.cumsum(~interval_valid, axis=0, dtype=numpy.int64),
        )
    )
    amihud_cumulative = numpy.vstack(
        (
            numpy.zeros((1, closes.shape[1]), dtype=numpy.float64),
            numpy.cumsum(daily_amihud, axis=0, dtype=numpy.float64),
        )
    )
    eligible = numpy.zeros_like(close_valid)
    amihud = numpy.full_like(closes, numpy.nan)
    momentum = numpy.full_like(closes, numpy.nan)
    history = protocol_module.MINIMUM_CONTIGUOUS_HISTORY_DAYS
    lookback = protocol_module.LIQUIDITY_LOOKBACK_DAYS
    for index in range(history - 1, len(market["dates"])):
        ...
        complete_intervals = (
            interval_invalid_cumulative[index + 1]
            - interval_invalid_cumulative[index + 1 - lookback]
            == 0
        )
        eligible[index] = complete_closes & complete_intervals
        columns = numpy.flatnonzero(eligible[index])
        if not len(columns):
            continue
        amihud[index, columns] = (
            amihud_cumulative[index + 1, columns]
            - amihud_cumulative[index + 1 - lookback, columns]
        ) / lookback
        momentum[index, columns] = (
            closes[index, columns]
            / closes[index - protocol_module.FORMATION_DAYS, columns]
            - 1.0
        )
        if not numpy.all(numpy.isfinite(amihud[index, columns])) or not numpy.all(
            numpy.isfinite(momentum[index, columns])
        ):
            raise DataQualityError("invalid liquid-winners causal feature")
    return {"eligible": eligible, "amihud": amihud, "momentum": momentum}
...
def target_weights(
    ...
    *,
    ...
    """Build causal winner and same-liquid benchmark targets."""

    date = market["dates"][index]
    if not is_rebalance_date(date):
        raise ValueError("liquid-winners target requires an anchored rebalance")
    ...
    eligible = [
        int(column)
        for column in numpy.flatnonzero(prepared["eligible"][index])
    ...
    liquid_count = max(
        1, math.floor(len(eligible) * protocol_module.LIQUID_FRACTION)
    )
    liquid = sorted(
        eligible,
        key=lambda value: (prepared["amihud"][index, value], symbols[value]),
    )[:liquid_count]
    winner_count = max(
        1, math.floor(len(liquid) * protocol_module.WINNER_FRACTION)
    )
    winners = sorted(
        liquid,
        key=lambda value: (-prepared["momentum"][index, value], symbols[value]),
    )[:winner_count]
    target[winners] = protocol_module.MAXIMUM_GROSS_EXPOSURE / len(winners)
    benchmark[liquid] = protocol_module.MAXIMUM_GROSS_EXPOSURE / len(liquid)
    if not numpy.isclose(
        numpy.sum(numpy.abs(target)),
        protocol_module.MAXIMUM_GROSS_EXPOSURE,
        atol=1e-12,
    ) or not numpy.isclose(
        numpy.sum(numpy.abs(benchmark)),
        protocol_module.MAXIMUM_GROSS_EXPOSURE,
        atol=1e-12,
    ):
        raise RuntimeError("liquid-winners target gross differs")
    return target, benchmark, {
        "status": "TARGET",
        "date": date.isoformat(),
        "eligible_assets": len(eligible),
        "liquid_assets": len(liquid),
        "winner_assets": len(winners),
        "liquid_symbols": [symbols[value] for value in liquid],
        "winner_symbols": [symbols[value] for value in winners],
        "liquid_amihud": [
            float(prepared["amihud"][index, value]) for value in liquid
        ],
        "winner_momentum": [
            float(prepared["momentum"][index, value]) for value in winners
        ],
    }
...
def _portfolio_contribution(
    ...
    return market_tools._portfolio_contribution(
...
def simulate_period(
    ...
    *,
    ...
    if end <= start or cost_multiplier < 1.0:
        raise ValueError("invalid liquid-winners simulation interval or cost")
    prepared = prepare_market(market) if prepared is None else prepared
    ...
    for index in indices:
        if is_rebalance_date(dates[index]):
            target, benchmark_target, audit = target_weights(
                market,
                index,
            ...
            liquid_assets.append(audit["liquid_assets"])
            winner_assets.append(audit["winner_assets"])
        market_tools._validate_outcome(
            market, index, target, benchmark_target
        )
        ...
        if (
            ...
            raise DataQualityError("invalid liquid-winners portfolio return")
    ...
    metrics, equity = market_tools._return_metrics(daily, outcome_dates)
    benchmark_metrics, benchmark_equity = market_tools._return_metrics(
    ...
    report = {
        ...
        **metrics,
        ...
        "annualized_excess_return_over_benchmark": (
        ...
        "minimum_eligible_assets_at_rebalance": min(
            eligible_assets, default=0
        ),
        "maximum_eligible_assets_at_rebalance": max(
            eligible_assets, default=0
        ),
        "minimum_liquid_assets_at_rebalance": min(liquid_assets, default=0),
        "maximum_liquid_assets_at_rebalance": max(liquid_assets, default=0),
        "minimum_winner_assets_at_rebalance": min(winner_assets, default=0),
        "maximum_winner_assets_at_rebalance": max(winner_assets, default=0),
...
def _training_gate(
    ...
    checks = {
        ...
        "minimum_active_rebalances": (
            report["active_rebalances"] >= gate["minimum_active_rebalances"]
        ),
        ...
        "minimum_annualized_excess_return_over_benchmark": (
            report["annualized_excess_return_over_benchmark"]
            >= gate["minimum_annualized_excess_return_over_benchmark"]
        ),
...
def _require_no_evaluation(output_root: pathlib.Path, protocol_sha256: str) -> None:
    matches = list(
        output_root.glob(f"liquid-winners-momentum-v1-{protocol_sha256[:12]}-*")
    )
    if matches:
        raise FileExistsError("official liquid-winners evaluation already exists")
...
def evaluate(
    ...
    folds = [
        simulate_period(market, start, end, prepared=prepared)
    ...
    experiment = output_root / (
        f"liquid-winners-momentum-v1-{protocol['protocol_sha256'][:12]}-"
        f"{experiment_key[:12]}"
    )
    ...
    try:
        ...
        market_tools._write_trajectory(
